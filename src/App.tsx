...
function App() {
  ...
  const [selectedImage, setSelectedImage] = useState<NormalizedImage | null>(
    null,
  );
  const [shortlistedImages, setShortlistedImages] = useState<NormalizedImage[]>(
    [],
  );
  const [savedGroups, setSavedGroups] = useState<SavedImageGroup[]>(
    getInitialSavedGroups,
  );
          ...
          {hasResults && (
            <div className="px-4 pb-24 sm:px-6">
              <ImageResults
                ...
                isShortlisted={(image) =>
                  shortlistedKeys.has(getImageKey(image))
                }
